...
func parser(bytesData []byte) ([]HostsFileLine, error) {
	...
	for idx, line := range fileLines {
		...
		// trim line
		curLine.trimed = strings.TrimSpace(line)
		...
		//TODO(areYouLazy): if the current hosts file has not been generated by this library,
		// we can find lines with multiple comments char (##, ###, ####, etc...) that are actually hosts lines
		// so we need to iterate and trim the prefix until `prefix != "#"`
		// only at this point we can try to parse the line as a hosts line
		// than, the parsed line will be rendered with only 1 '#'
